...
ffmpeg_conv_audio_format = 'ffmpeg -i {input_file} -itsoffset 0.150 -i {input_file} -vcodec copy -acodec copy -map 0:0 -map 1:1 {op_filename}'


for f in glob.glob('*_ss_pcm.avi'):
    this_audio_conv = ffmpeg_conv_audio_format.format(
        input_file=f,
        op_filename=f.replace('_pcm.avi', '_pcm_del.avi')
        )
    print(this_audio_conv)
    os.system(this_audio_conv)
