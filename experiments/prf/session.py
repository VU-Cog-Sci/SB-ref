...
class PRFSession(EyelinkSession):
    ...
    def prepare_trials(self):
        ...
        for argument in ['mask_type', 'vertical_stim_size', 'horizontal_stim_size', 'x_offset',
                         'bar_width_ratio', 'num_elements', 'eyelink_calib_size', 'eyelink_nr_calib_points'
        ...
        if standard_parameters['mask_type'] == 0:
            standard_parameters['horizontal_stim_size'] = self.size[1]/self.size[0]
         #Set arguments from config file or kwargs
        ...
        for i in range(len(self.stim_bool)):
            if self.stim_bool[i] == 0:
                self.bar_pass_durations.append(self.empty_bar_pass_in_TR * self.TR)
            else:
                if self.stim_direction_indices[i] in (2,6): # EW-WE:
                    self.bar_pass_durations.append(self.horizontal_bar_pass_in_TR * self.TR)
        ...
        self.phase_durations = np.array([
            ...
            self.PRF_ITI_in_TR * self.TR ])# for stim_dur in self.stim_durations])    # ITI
        ...
        # mask
        if self.mask_type ==1:
            draw_screen_space = [self.screen_pix_size[0]*self.horizontal_stim_size,self.screen_pix_size[1]*self.vertical_stim_size]
            ...
            mask = filters.makeMask(matrixSize = self.screen_pix_size[0], shape='raisedCosine', radius=self.vertical_stim_size*self.screen_pix_size[1]/self.screen_pix_size[0]/2, center=(0.0, 0.0), range=[1, -1], fringeWidth=0.1 )
