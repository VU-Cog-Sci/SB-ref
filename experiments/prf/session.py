...
class PRFSession(EyelinkSession):
    def __init__(self, subject_initials, index_number, scanner, tracker_on, **kwargs):
        super(PRFSession, self).__init__(subject_initials=subject_initials, index_number=index_number, scanner=scanner, tracker_on=tracker_on, **kwargs)
        ...
        if tracker_on:
           ...
           # how many points do we want:
            ...
            # create 4 x levels:
            width = standard_parameters['eyelink_calib_size'] * \
                0.75 * DISPSIZE[1]
            x_start = (DISPSIZE[0]-width)/2
            x_end = DISPSIZE[0]-(DISPSIZE[0]-width)/2
            x_range = np.linspace(x_start, x_end, 5) + \
